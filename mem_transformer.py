...
class PositionalEmbedding(nn.Module):
    ...
    def forward(self, pos_seq, bsz=None):
        # renturn embedding with length of pos_seq
...
class RelMultiHeadAttn(nn.Module):
    ...
    def _rel_shift(self, x, zero_triu=False):# shift of RelPartialLearnableBecoderLayer ???
        #print("#### _rel_shift ####")
...
class RelPartialLearnableMultiHeadAttn(RelMultiHeadAttn):
    ...
    def forward(self, w, r, r_w_bias, r_r_bias, attn_mask=None, mems=None):
        ...
        klen = w_head_k.size(0) # = memlen+qlen
        ...
        rw_head_q = w_head_q + r_w_bias  # u                                    # qlen x bsz x n_head x d_head
        AC = torch.einsum('ibnd,jbnd->ijbn', (rw_head_q, w_head_k))             # qlen x klen x bsz x n_head
        # AC= a + c

        rr_head_q = w_head_q + r_r_bias  # v
        BD = torch.einsum('ibnd,jnd->ijbn', (rr_head_q, r_head_k))              # qlen x klen x bsz x n_head
        # BD= b + d
        # RelMultiHeadAttn rel_shift
...
class AdaptiveEmbedding(nn.Module):
    def __init__(self, n_token, d_embed, d_proj, cutoffs, div_val=1, 
        ...
        # print("#######  AdaptiveEmbedding Start  ########")
        # print("n_token(n_token)",n_token)
        # print("d_embed(d_embed)",d_embed)
        # print("d_proj(d_model)",d_proj)
        # print("cutoffs(cutoffs)",cutoffs)
        # print("div_val(div_val)",div_val)
        # print("sample_softmax(div_val)",sample_softmax)
        # print("#######  AdaptiveEmbedding End  ########")
        ...
        if div_val == 1:
            ...
            if d_proj != d_embed:
                #print("self.emb_projs.append")
                self.emb_projs.append(nn.Parameter(torch.Tensor(d_proj, d_embed)))
        else:
            #print("### div_val != 1  ###")
            #print("len(self.cutoffs) = ",len(self.cutoffs))
...
class MemTransformerLM(nn.Module):
    ...
    def init_mems(self):
        if self.mem_len > 0:
            ...
            #print("initial len(mems) = ", len(mems))
            #print(mems)
            return mems
        else:
            return None
    ...
    def _update_mems(self, hids, mems, qlen, mlen):
        ...
        # print("######  len(new_mems) = {} ######".format(len(new_mems)))
        # print("######  len(new_mems[0]) = {} ######".format(len(new_mems[0])))
        # print("######  len(new_mems[0][0]) = {} ######".format(len(new_mems[0][0])))
        # print("######  len(new_mems[0][0][0]) = {} ######".format(len(new_mems[0][0][0])))
        # print(new_mems[0][0])
        # print(new_mems[0].shape)
        
        return new_mems

    def _forward(self, dec_inp, mems=None):
        # dec_inp = data ,mems=mems
        # print("dec_inp.shape = ",dec_inp.shape)
        ...
        #print("self.same_length=",self.same_length)
        ...
        if self.attn_type == 0: # default
            ...
            #print("pos_emb.shape1",self.pos_emb)
            #get embedding of length pos_seq
    ...
    def forward(self, data, target, *mems, model_type = "training"):
        ...
        if self.sample_softmax > 0 and model_type == "training":
            ...
            output = self.crit(pred_hid.view(-1, pred_hid.size(-1)), target ,False, model_type)
            #outputTest=output.view(tgt_len,8,-1)
            #loss = loss.view(tgt_len, -1)
            return [output] + new_mems
        else:
            loss,output = self.crit(pred_hid.view(-1, pred_hid.size(-1)), target.view(-1))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--d_embed', type=int, default=201, help='')
    parser.add_argument('--d_inner', type=int, default=202, help='')
